...
class ProfileTestClass(TestCase):
    from django.contrib.auth.models import User
    def setUp(self):
        self.user = User(username='briankiplanagat')
        self.user.save()
        self.profile = Profile(id=1,user=self.user,photo='download.jpeg',bio='My name is briankiplangat', name='person')
...
class ImageTestClass(TestCase):
    def setUp(self):
        self.profile = Profile(user=User(username='Japhet'))
    ...
    def test_update_image_caption(self):
        ...
        self.assertEqual(image.caption,'Yeah')
